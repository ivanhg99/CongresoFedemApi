<?php

namespace App\Entity;

use Symfony\Component\Serializer\Annotation\Groups;
use Doctrine\ORM\Mapping as ORM;

/**
 * Actividad
 *
 * @ORM\Table(name="actividad", indexes={@ORM\Index(name="fk_actividad_congreso1_idx", columns={"congreso_id"})})
 * @ORM\Entity
 */
class Actividad
{
    /**
     * @var int
     * @Groups("actividad")
     * @ORM\Column(name="id", type="integer", nullable=false, options={"unsigned"=true})
     * @ORM\Id
     * @ORM\GeneratedValue(strategy="IDENTITY")
     */
    private $id;

    /**
     * @var string
     * @Groups("actividad")
     * @ORM\Column(name="nombre", type="string", length=45, nullable=false)
     */
    private $nombre;

    /**
     * @var string
     * @Groups("actividad")
     * @ORM\Column(name="descripcion", type="string", length=255, nullable=false)
     */
    private $descripcion;

    /**
     * @var string
     * @Groups("actividad")
     * @ORM\Column(name="imagen", type="string", length=255, nullable=false)
     */
    private $imagen;

    /**
     * @var Congreso
     * @Groups("actividad")
     * @ORM\ManyToOne(targetEntity="Congreso")
     * @ORM\JoinColumns({
     *   @ORM\JoinColumn(name="congreso_id", referencedColumnName="id")
     * })
     */
    private $congreso;

    /**
     * @var int
     * @Groups("evento")
     * @ORM\Column(name="dia", type="integer", nullable=false, options={"unsigned"=true})
     */
    private $dia;

    /**
     * @var int
     * @Groups("evento")
     * @ORM\Column(name="mes", type="integer", nullable=false)
     */
    private $mes;


    /**
     * @var \DateTime
     * @Groups("actividad")
     * @ORM\Column(name="hora_inicio", type="time", nullable=false)
     */
    private $horaInicio;

    /**
     * @var \DateTime
     * @Groups("actividad")
     * @ORM\Column(name="hora_fin", type="time", nullable=false)
     */
    private $horaFin;

    /**
     * @var \Doctrine\Common\Collections\Collection
     * @Groups("actividad")
     * @ORM\ManyToMany(targetEntity="Ajeno", inversedBy="actividad")
     * @ORM\JoinTable(name="ajeno_asiste_actividad",
     *   joinColumns={
     *     @ORM\JoinColumn(name="actividad_id", referencedColumnName="id")
     *   },
     *   inverseJoinColumns={
     *     @ORM\JoinColumn(name="ajeno_id_ajeno", referencedColumnName="id_ajeno")
     *   }
     * )
     */
    private $ajenoIdAjeno = array();

    /**
     * Constructor
     */
    public function __construct()
    {
        $this->ajenoIdAjeno = new \Doctrine\Common\Collections\ArrayCollection();
    }

    /**
     * @return int
     */
    public function getId(): int
    {
        return $this->id;
    }

    /**
     * @return string
     */
    public function getNombre(): string
    {
        return $this->nombre;
    }

    /**
     * @return string
     */
    public function getDescripcion(): string
    {
        return $this->descripcion;
    }

    /**
     * @return string
     */
    public function getImagen(): string
    {
        return $this->imagen;
    }

    /**
     * @return Congreso
     */
    public function getCongreso(): Congreso
    {
        return $this->congreso;
    }

    /**
     * @return \Doctrine\Common\Collections\ArrayCollection|\Doctrine\Common\Collections\Collection
     */
    public function getAjenoIdAjeno()
    {
        return $this->ajenoIdAjeno;
    }

    /**
     * @param string $nombre
     */
    public function setNombre(string $nombre): void
    {
        $this->nombre = $nombre;
    }

    /**
     * @param string $descripcion
     */
    public function setDescripcion(string $descripcion): void
    {
        $this->descripcion = $descripcion;
    }

    /**
     * @param string $imagen
     */
    public function setImagen(string $imagen): void
    {
        $this->imagen = $imagen;
    }

    /**
     * @param Congreso $congreso
     */
    public function setCongreso(Congreso $congreso): void
    {
        $this->congreso = $congreso;
    }

    /**
     * @param \Doctrine\Common\Collections\ArrayCollection|\Doctrine\Common\Collections\Collection $ajenoIdAjeno
     */
    public function setAjenoIdAjeno($ajenoIdAjeno): void
    {
        $this->ajenoIdAjeno = $ajenoIdAjeno;
    }

    /**
     * @return \DateTime
     */
    public function getHoraInicio(): \DateTime
    {
        return $this->horaInicio;
    }

    /**
     * @param \DateTime $horaInicio
     */
    public function setHoraInicio(\DateTime $horaInicio): void
    {
        $this->horaInicio = $horaInicio;
    }

    /**
     * @return \DateTime
     */
    public function getHoraFin(): \DateTime
    {

        return $this->horaFin;
    }

    /**
     * @param \DateTime $horaFin
     */
    public function setHoraFin(\DateTime $horaFin): void
    {
        $this->horaFin = $horaFin;
    }

    /**
     * @return int
     */
    public function getDia(): int
    {
        return $this->dia;
    }

    /**
     * @param int $dia
     */
    public function setDia(int $dia): void
    {
        $this->dia = $dia;
    }

    /**
     * @return int
     */
    public function getMes(): int
    {
        return $this->mes;
    }

    /**
     * @param int $mes
     */
    public function setMes(int $mes): void
    {
        $this->mes = $mes;
    }

}
